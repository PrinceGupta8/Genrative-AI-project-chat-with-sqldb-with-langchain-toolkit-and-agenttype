...
sql_opt=['Sqlite3 database student','MySQL database']
selected_option=st.sidebar.radio('Select Database',options=sql_opt)

LOCALDB="USE_LOCALDB"
MYSQL="USE_MYSQL"

if sql_opt.index(selected_option)==1:
    db_uri=MYSQL
    ...
    mysql_db=st.sidebar.text_input(label='MYSQL database')
else:
    db_uri=LOCALDB
...
if not db_uri:
    st.info('Please enter database information and uri_db')
